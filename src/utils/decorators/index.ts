...
export function OnEvent(event: string, options?: { async: boolean }) {
  ...
  /**
 * Decorator function that subscribes the target method to the given Socket event.
 *
 * @param {string} event - The event to listen for.
 * @return {Function} - A decorator function that subscribes the target method.
 */
export function SubscribeMessage(event: string) {  
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        const originalMethod = descriptor.value;         
        // getSocketIo().on(event, (message: any) => {
        //     originalMethod.apply(target, [message]);
        // });
    };
}
...
/**
 * Parameter decorator that assigns 'payload' to the specified parameter index
 * of the target method's metadata.
 * 
 * This decorator updates the 'design:paramtypes' metadata of the target method
 * by marking the given parameter index as 'payload'. This can be useful for
 * identifying payload parameters in methods for further processing or validation.
 * 
 * @return {Function} A decorator function that modifies the parameter metadata.
 */
export function Payload() {
    return function (target: any, propertyKey: string | symbol, parameterIndex: number) {
        const existingMetadata = Reflect.getOwnMetadata('design:paramtypes', target, propertyKey) || [];
        existingMetadata[parameterIndex] = 'payload';
        Reflect.defineMetadata('design:paramtypes', existingMetadata, target, propertyKey);
    };
}